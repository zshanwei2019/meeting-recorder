...
> **首次下载（约 2.9GB）在启动后台线程进行**：下载期间顶部状态栏会显示“正在准备语音模型…”；
> 若下载失败（断网/超时/磁盘不足等），界面右上角会**弹出明确的错误提示条**（不再静默）。
>
> 仍无法下载时的排查：检查 `~/.cache/modelscope/models` 是否包含上述模型目录；
> 或**在前台运行 `python app.py` 观察下载日志**，也可点界面“环境自检”查看各模型状态。
